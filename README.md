...
-- אפשרות 2: שימוש ב-IN (הצורה היעילה יותר)
...
-- אפשרות 2: שימוש ב-JOIN ו-GROUP BY
SELECT p.first_name, p.last_name, p.phone, 
...
-- אפשרות 2: שימוש ב-EXISTS (הצורה היעילה יותר)
...
1. נכניס מטופל ראשון עם כתובת מייל מסוימת (הפעולה תצליח).  
...
check constraint (CHK_PATIENT_GENDER) violated
